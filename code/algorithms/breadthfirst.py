...
def breadth_first_solution(graph, scale, beam=5):
    ...
    while len(routemap.routes) != MAX_ROUTES:
        ...
        options = [Route(MAX_TIME, start_station=station) for station in graph.stations.values()]
        best_option = (None, 0)
        ...
        while options:
            ...
            options = [route[0] for route in sorted(scored_routes, key=lambda x: x[1], reverse = True)[:beam]]
